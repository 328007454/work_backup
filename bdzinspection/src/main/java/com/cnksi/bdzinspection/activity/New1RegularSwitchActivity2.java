...
//        }
    ...
    private boolean saveStepStatus(DbModel model) {
        ...
        try {
            item = CustomApplication.getDbUtils().findFirst(
        ...
        try {
            CustomApplication.getDbUtils().saveOrUpdate(item);
        ...
        switch (msg.what) {
            case LOAD_DATA:
                if (mRegularSwitchAdapter == null) {
                    mRegularSwitchAdapter = new New1RegularSwitchListAdapter2(currentActivity);
...
//                if ("maintenance_xdcdyjc".equalsIgnoreCase(currentInspectionType)) {
    ...
    private void saveOrUpdateInputValue(final int messageCode) {
        mFixedThreadPoolExecutor.execute(new Runnable() {
            @Override
            public void run() {
//                if (!TextUtils.isEmpty(PreferencesUtils.getString(currentActivity, SwitchMenu.REPSWITCHOVERID, ""))) {
//                    report.repSwithoverId = PreferencesUtils.getString(currentActivity, SwitchMenu.REPSWITCHOVERID, "");
                try {
                    CustomApplication.getDbUtils().saveOrUpdate(report);
                } catch (DbException e) {
                    e.printStackTrace();
