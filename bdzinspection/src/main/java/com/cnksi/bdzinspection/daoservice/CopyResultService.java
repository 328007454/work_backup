...
public class CopyResultService {
    ...
    public void saveOrUpdate(CopyResult copyResult) {
        ...
        try {
            if (TextUtils.isEmpty(copyResult.id)) {
                ...
                XunshiApplication.getDbUtils().save(copyResult);
            } else {
                XunshiApplication.getDbUtils().replace(copyResult);
            }
        } catch (DbException e) {
            e.printStackTrace();
        }
    ...
    public CopyResult getResult(String reportId, String itemId) {
        try {
            Selector selector = XunshiApplication.getDbUtils().selector(CopyResult.class).where(CopyResult.REPORTID, "=", reportId).and(CopyResult.ITEM_ID, "=", itemId).orderBy(CopyResult.UPDATE_TIME);

            return (CopyResult) selector.findFirst();
        } catch (DbException e) {
            e.printStackTrace();
        }
        return null;
    ...
    public List<CopyResult> getResultList(String bdzid, String reportId, String deviceId, boolean inReport) {
        try {
            ...
            Selector selector = XunshiApplication.getDbUtils().selector(CopyResult.class).where(CopyResult.DEVICEID, "=", deviceId).and(CopyResult.BDZID, "=", bdzid);
            if (inReport) {
    ...
    public long getReportCopyCount(String reportId) {
        ...
        sqlInfo.addBindArg(new KeyValue("", reportId));
        try {
            DbModel dbMode = XunshiApplication.getDbUtils().findDbModelFirst(sqlInfo);
    ...
    public void clearNULLData() {
        ...
        try {
            XunshiApplication.getDbUtils().execNonQuery(sql);
        } catch (DbException e) {
            e.printStackTrace();
        }
    }
    ...
    public List<CopyResult> getResultList(String bdzid, String reportId, String deviceId, boolean inReport, String type) {
        try {
            ...
            Selector selector = XunshiApplication.getDbUtils().selector(CopyResult.class).where(CopyResult.DEVICEID, "=", deviceId).and(CopyResult.REPORTID, operate, reportId).expr(" and " + CopyItem.TYPE_KEY + " in('" + type + "') and create_time in ( select max(create_time) from copy_result cr where cr.bdzid = '" + bdzid + "' and cr.deviceid = '" + deviceId + "' group by item_id)  group by " + CopyResult.ITEM_ID + " order by " + CopyResult.UPDATE_TIME + "," + CopyResult.CREATE_TIME);
            return selector.findAll();
        } catch (DbException e) {
            e.printStackTrace();
        }
    ...
    public List<CopyResult> findAllMaintenanceCopyResult(String reportId, String currentInspectionType, String currentBdzId) throws DbException {
        ...
        copyResults = XunshiApplication.getDbUtils().selector(CopyResult.class).where(CopyResult.TYPE_KEY, "=", currentInspectionType).and(CopyResult.REPORTID, "=", reportId).and(CopyResult.BDZID, "=", currentBdzId).and(CopyResult.DLT, "=", "0").findAll();
        return copyResults;
    }
}
