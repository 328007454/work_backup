...
public class DefectRecord extends BaseModel {
     ...
     * @param pics        缺陷图片
     */
    public DefectRecord(String reportid, String bdzid, String bdz, String spid, String spname, String deviceid,
                        String device, String duid, String duname, String defectlevel, String description, String standid,
                        String pics) {
     ...
     */
    public DefectRecord(String reportid, String bdzid, String bdz, String spid, String spname, String deviceid,
                        String device, String duid, String duname, String val, String unit) {
        this.reportid = reportid;
        this.bdzid = bdzid;
        this.bdz = bdz;
        this.spid = spid;
    ...
    public DefectRecord(DefectRecord mDefectRecord) {
        this(mDefectRecord.reportid, mDefectRecord.bdzid, mDefectRecord.bdz, mDefectRecord.spid, mDefectRecord.spname,
                mDefectRecord.deviceid, mDefectRecord.devcie, mDefectRecord.duid, mDefectRecord.duname, "", "");
    }
    ...
    public void initArresterActionValue() {
        if (this.duname != null && this.devcie.contains("避雷器")) {
            if (this.description != null && this.description.contains("动作")) {

                this.dzcs = CalcUtils.sub(val, oldval);
                SqlInfo sql = new SqlInfo();
                sql.setSql("SELECT dzcs FROM defect_record where deviceid=? and standid=? and bdzid=? and dlt=0 ORDER BY discovered_date DESC LIMIT 1");
                sql.addBindArg(new KeyValue(DEVICEID, this.deviceid));
