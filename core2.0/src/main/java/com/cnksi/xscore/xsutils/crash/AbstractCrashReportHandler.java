...
public abstract class AbstractCrashReportHandler implements CrashListener {
	 ...
	 */
	@SuppressWarnings("deprecation")
	public static ErrorLogBean buildBodyForBean(Context context) {

		ErrorLogBean bean = new ErrorLogBean();
		bean.date_time = DateUtils.getCurrentLongTime();
		PackageManager pm = context.getPackageManager();
		ApplicationInfo ai = context.getApplicationInfo();
		bean.application = pm.getApplicationLabel(ai).toString();
		try {
			PackageInfo pi = pm.getPackageInfo(ai.packageName, 0);
			bean.version_code = String.valueOf(pi.versionCode);
			bean.version_name = pi.versionName;
		} catch (PackageManager.NameNotFoundException e) {
			e.printStackTrace();
		}
		bean.board = Build.BOARD;
		bean.cpu_abi = Build.CPU_ABI;
		bean.cpu_abi2 = Build.CPU_ABI2;
		bean.device = Build.DEVICE;
		bean.manufacturer = Build.MANUFACTURER;
		bean.product = Build.PRODUCT;
		return bean;
	}
	 ...
	 * @return
	 */
	public static ErrorLogBean buildBodyForBean() {
		return buildBodyForBean(mContext);
	}
}
