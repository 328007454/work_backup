...
public class AllDeviceListActivity extends BaseActivity implements DeviceExpandabelListAdapter.OnAdapterViewClickListener {
	...
	private void initData() {
		...
		mFixedThreadPoolExecutor.execute(new Runnable() {
			...
			public void run() {
				try {
					List<Spacing> mSpacingList = db.selector(Spacing.class).where(Spacing.BDZID, "=", currentBdzId).
							expr("and spid in (select distinct(spid) spid from device where device_type = '" + currentFunctionModel.name() + "' and bdzid = '" + currentBdzId + "')").
					...
					if (mSpacingList != null) {
						...
						for (Spacing mSpacing : mSpacingList) {
							SqlInfo sqlInfo = new SqlInfo(
									"select * from device d where d.spid='"+mSpacing.spid+"' and d.device_type='"+currentFunctionModel.name()+"'  group by d.deviceid order by sort");
							List<DbModel> mDeviceList =db.findDbModelAll(sqlInfo);
