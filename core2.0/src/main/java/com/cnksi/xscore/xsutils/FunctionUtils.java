...
public class FunctionUtils {
	...
	public static void takePicture(Activity activity, String currentImageName, String defectImagePath) {
		takePicture(activity, currentImageName, defectImagePath, BaseCoreActivity.ACTION_IMAGE);
	}
	...
	public static void takePicture(Activity activity, String currentImageName, String imageFolder, int requestCode) {
		...
		values.put(Images.Media.TITLE, imageFile.getAbsolutePath());
		values.put("_data", imageFile.getAbsolutePath());
		values.put(Images.Media.PICASA_ID, imageFile.getAbsolutePath());
		values.put(Images.Media.DISPLAY_NAME, imageFile.getAbsolutePath());
		values.put(Images.Media.DESCRIPTION, imageFile.getAbsolutePath());
		values.put(Images.ImageColumns.BUCKET_DISPLAY_NAME, imageFile.getAbsolutePath());
		Uri photoUri = activity.getContentResolver().insert(Images.Media.EXTERNAL_CONTENT_URI, values);
		intent.putExtra(Images.Media.ORIENTATION, 0);
	...
	public static void takePictureFromFragment(Fragment fragment, String currentImageName, String defectImagePath) {
		takePictureFromFragment(fragment, currentImageName, defectImagePath, BaseCoreFragment.ACTION_IMAGE);
	}
	...
	public static void takePictureFromFragment(Fragment fragment, String currentImageName, String imageFolder, int requestCode) {
		...
		values.put(Images.Media.DESCRIPTION, imageFile.getAbsolutePath());
		values.put(Images.ImageColumns.BUCKET_DISPLAY_NAME, imageFile.getAbsolutePath());
		Uri photoUri = fragment.getActivity().getContentResolver().insert(Images.Media.EXTERNAL_CONTENT_URI, values);
		intent.putExtra(Images.Media.ORIENTATION, 0);
		intent.putExtra(MediaStore.EXTRA_OUTPUT, photoUri);
	...
	public static void startBaiduNavi(Context mContext, String endLatitude, String endLongitude, String address) {
		try {
			...
			if (AppUtils.isAppInstalled(mContext, "com.baidu.BaiduMap")) {
				...
				String intentStr = "intent://map/marker?location=" + endLatitude + "," + endLongitude + "&title=" + address + "&content=" + address + "&src=" + packageName + "|" + appName
