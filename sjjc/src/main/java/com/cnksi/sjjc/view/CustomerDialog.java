...
public class CustomerDialog {
    ...
    public static Dialog showSelectDialog(Activity context, String title, String toast, final DialogClickListener dialogClickListener, int confirmTextId, int canceltextId, boolean isPms) {
        return ShowDialog(context, title, toast, dialogClickListener, SELECT_DIALOG, context.getResources().getString(confirmTextId), context.getResources().getString(canceltextId), isPms);
    }
    ...
    private static Dialog ShowDialog(final Activity context, String title, String toast, final DialogClickListener dialogClickListener, final int DialogType, String confirmText, String cancelText) {
        if (context != null && !context.isDestroyed()) {
            final Dialog mCustomDialog = new Dialog(context, R.style.DialogStyle);
    ...
    private static Dialog ShowDialog(final Activity context, String title, String toast, final DialogClickListener dialogClickListener, final int DialogType, String confirmText, String cancelText, boolean isPms) {
        if (context != null && !context.isDestroyed()) {
            final Dialog mCustomDialog = new Dialog(context, R.style.DialogStyle);
            mCustomDialog.setCancelable(false);
            View view = LayoutInflater.from(context).inflate(R.layout.dialog_customer, null);
    ...
    private static Dialog ShowDialog(Activity context, String[] items, boolean isCanceledOnTouchOutside, final DialogItemClickListener dialogClickListener) {
        if (context != null && !context.isDestroyed()) {
            ...
            for (int i = 0; i < length; i++) {
                LayoutParams params1 = new LayoutParams(LayoutParams.MATCH_PARENT, context.getResources().getDimensionPixelOffset(R.dimen.dialog_button_minheight));
