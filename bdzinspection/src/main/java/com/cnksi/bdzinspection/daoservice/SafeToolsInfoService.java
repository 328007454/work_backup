...
public class SafeToolsInfoService extends BaseService {
    ...
    public List<DbModel> findAllTools(String dept, String keyWord, String bdzId) {
        ...
        try {
            toolsInfors = CustomApplication.getDbUtils().findDbModelAll(new SqlInfo(sql));
    ...
    public List<DbModel> findAllTools(String dept) {
        ...
        try {
            return CustomApplication.getDbUtils().findDbModelAll(new SqlInfo(sql, dept));
        } catch (DbException e) {
            e.printStackTrace();
        }
    ...
    public List<OperateToolResult> finAllResults(String deptId, String toolId, String bdzId) {
        try {
            return CustomApplication.getDbUtils().findAll(Selector.from(OperateToolResult.class).where(OperateToolResult.GQJ_ID, "=", toolId).and(OperateToolResult.DLT, "=", "0").and(OperateToolResult.OPERATION, "=", Config.ToolStatus.test.name()).orderBy(OperateToolResult.CREATETIME, true));
        } catch (DbException e) {
            e.printStackTrace();
    ...
    public Map<String, String> countRemindGroupByBdz(String deptId) {
        ...
        try {
            List<DbModel> models = CustomApplication.getDbUtils().findDbModelAll(new SqlInfo("SELECT bdz_id,bdz_name,count(*) AS c  FROM gqj_info where dept_id=? and dlt=0 and status not in('"
